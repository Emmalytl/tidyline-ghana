...
import {
  ...
  XCircle, Calculator, Banknote, Receipt, TrendingDown
} from "lucide-react";
...
function Accounting({ bookings, staff }) {
  ...
  async function loadExpenses() {
    ...
    const { data, error } = await supabase.from("expenses").select("*").gte("expense_date", `${month}-01`).lt("expense_date", nextMonth).order("expense_date", { ascending: false });
  ...
  async function addExpense(e) {
    ...
    const payload = {
      ...
      booking_id: newExpense.booking_id || null,
    };
    const { data, error } = await supabase.from("expenses").insert(payload).select().single();
    if (error) { alert(error.message); return; }
    setExpenses(prev => [data, ...prev]);
    setNewExpense({ expense_date: new Date().toISOString().slice(0,10), category: "Supplies", description: "", amount: "", booking_id: "" });
  }
  ...
  const netProfit = businessAfterAdminAndPayroll - totalExpenses;
  ...
  const EXPENSE_CATEGORIES = ["Supplies","Transport","Equipment","Rent","Utilities","Marketing","Other"];
  return <div className="ops-page">
    ...
    <div className="ops-stat-grid">
      ...
      <Stat icon={Receipt} label="Other expenses" value={money(totalExpenses)} detail={`${expenses.length} logged this month`} />
    </div>
    <div className="ops-grid-2">
    ...
    <section className="ops-card"><div className="ops-card-head"><div><h2>Expenses</h2><p>Business costs this month — supplies, transport, equipment, and anything else spent to run jobs. Optionally link one to a booking.</p></div></div>
      <form className="ops-inline-form" onSubmit={addExpense}>
        <input type="date" value={newExpense.expense_date} onChange={e=>setNewExpense({...newExpense, expense_date:e.target.value})}/>
        <select value={newExpense.category} onChange={e=>setNewExpense({...newExpense, category:e.target.value})}>{EXPENSE_CATEGORIES.map(c=><option key={c}>{c}</option>)}</select>
        ...
        <select value={newExpense.booking_id} onChange={e=>setNewExpense({...newExpense, booking_id:e.target.value})}><option value="">No linked booking</option>{activeBookings.map(b=><option key={b.id} value={b.id}>{b.booking_ref} — {b.name}</option>)}</select>
        <input type="number" step="0.01" min="0" placeholder="Amount (GH₵)" value={newExpense.amount} onChange={e=>setNewExpense({...newExpense, amount:e.target.value})}/>
        <button className="ops-primary" type="submit"><Plus/> Add expense</button>
      </form>
